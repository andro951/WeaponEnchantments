...
namespace WeaponEnchantments
{
    public static class PlayerFunctions
    {
        public static void CheckWeapon(this Item newItem, ref Item oldItem, Player player, int slot)
        {
            ...
            if (checkWeapon)
            {
                if(UtilityMethods.debugging) ($"\\/CheckWeapon({newItem.S()}, {oldItem.S()}, player: {player.S()}, slot: {slot}) ").Log();
    ...
    public class WEPlayer : ModPlayer
    {
        ...
        public override bool ShiftClickSlot(Item[] inventory, int context, int slot)
        {
            if (usingEnchantingTable)
            {
                ...
                for (int j = 0; j < EnchantingTable.maxItems; j++)
                {
                    ...
                    {
                        stop = true;
                        enchantingTableSlotItem = enchantingTableUI.itemSlotUI[j].ItemInSlot;
                    }
                }//Check itemSlot(s)
                for (int j = 0; j < EnchantingTable.maxEnchantments && !stop; j++)
                {
                    ...
                    {
                        stop = true;
                        enchantingTableSlotItem = enchantingTableUI.enchantmentSlotUI[j].ItemInSlot;
                    }
                ...
                for (int j = 0; j < EnchantingTable.maxEssenceItems && !stop; j++)
                {
                    ...
                    {
                        stop = true;
                        enchantingTableSlotItem = enchantingTableUI.essenceSlotUI[j].ItemInSlot;
                    }
                }//Check essenceSlots
                if (stop)
                {
        ...
        public void CheckShiftClickValid(ref Item item, bool moveItem = false)
        {
            ...
            if (Main.mouseItem.IsAir)
            {
                ...
                {
                    Item tableItem = enchantingTableUI.itemSlotUI[0].ItemInSlot;
                    if (item.type == PowerBooster.ID && !enchantingTableUI.itemSlotUI[0].ItemInSlot.IsAir && !enchantingTableUI.itemSlotUI[0].ItemInSlot.G().PowerBoosterInstalled)
                    {
                        if (moveItem)
                        {
                            enchantingTableUI.itemSlotUI[0].ItemInSlot.G().PowerBoosterInstalled = true;
                            if (item.stack > 1)
                                item.stack--;
                            else
                                item = new Item();
                    ...
                    else
                    {
                            ...
                            if (enchantingTableUI.itemSlotUI[i].Valid(item))
                            {
                                if (!item.IsAir)
                                {
                                    ...
                                    if (!doNotSwap)
                                    {
                                        if (moveItem)
                                        {
                                            enchantingTableUI.itemSlotUI[i].ItemInSlot = item.Clone();
                        ...
                        if (!valid)
                        {
                            ...
                            {
                                int uniqueItemSlot = WEUIItemSlot.FindSwapEnchantmentSlot(enchantment, enchantingTableUI.itemSlotUI[0].ItemInSlot);
                                for (int i = 0; i < EnchantingTable.maxEnchantments; i++)
                                {
                                    if (enchantingTableUI.enchantmentSlotUI[i].Valid(item))
                                    {
                                        if (!item.IsAir)
                                        {
                                            if (enchantingTableUI.enchantmentSlotUI[i].ItemInSlot.IsAir && uniqueItemSlot == -1)
                                            {
                                                if (moveItem)
                                                {
                                                    int s = i;
                                                    if (enchantment.IsUtility && enchantingTableUI.enchantmentSlotUI[4].ItemInSlot.IsAir && (WEMod.IsWeaponItem(tableItem) || WEMod.IsArmorItem(tableItem)))
                                                    {
                                                        s = 4;
                                                    }
                                                    enchantingTableUI.enchantmentSlotUI[s].ItemInSlot = item.Clone();
                                                    enchantingTableUI.enchantmentSlotUI[s].ItemInSlot.stack = 1;
                                                    if (item.stack > 1)
                                                    {
                                                        item.stack--;
                                                    }
                                            ...
                                            else if (uniqueItemSlot != -1 && enchantingTableUI.enchantmentSlotUI[i].CheckUniqueSlot(enchantment, uniqueItemSlot) && item.type != enchantingTableUI.enchantmentSlotUI[i].ItemInSlot.type)
                                            {
                                                if (moveItem)
                                                {
                                                    Item returnItem = enchantingTableUI.enchantmentSlotUI[i].ItemInSlot.Clone();
                                                    enchantingTableUI.enchantmentSlotUI[i].ItemInSlot = item.Clone();
                                                    enchantingTableUI.enchantmentSlotUI[i].ItemInSlot.stack = 1;
                                                    if (!returnItem.IsAir)
                                                    {
                                                        if (item.stack > 1)
                                                        {
                            ...
                            for (int i = 0; i < EnchantingTable.maxEssenceItems; i++)
                            {
                                if (enchantingTableUI.essenceSlotUI[i].Valid(item))
                                {
                                    if (!item.IsAir)
                                    {
                                        ...
                                        if (enchantingTableUI.essenceSlotUI[i].ItemInSlot.IsAir)
                                        {
                                            if (moveItem)
                                            {
                                                enchantingTableUI.essenceSlotUI[i].ItemInSlot = item.Clone();
                                                item = new Item();
                                            }
                                            canTransfer = true;
                                        }//essence slot empty
                                        ...
                                        {
                                            if (enchantingTableUI.essenceSlotUI[i].ItemInSlot.stack < EnchantmentEssence.maxStack)
                                            {
                                                if (moveItem)
                                                {
                                                    int ammountToTransfer;
                                                    if (item.stack + enchantingTableUI.essenceSlotUI[i].ItemInSlot.stack > EnchantmentEssence.maxStack)
                                                    {
                                                        ammountToTransfer = EnchantmentEssence.maxStack - enchantingTableUI.essenceSlotUI[i].ItemInSlot.stack;
                                                    ...
                                                    enchantingTableUI.essenceSlotUI[i].ItemInSlot.stack += ammountToTransfer;
                                                }
                                                canTransfer = true;
                                            }
        ...
        public override void PostUpdate()
        {
            //if (Main.netMode < NetmodeID.Server)
            {
                ...
                {
                    if(UtilityMethods.debugging) ($"\\/Start hoverItem check").Log();
                    Item newItem = null;
                    if (usingEnchantingTable && UtilityMethods.IsSameEnchantedItem(enchantingTableUI.itemSlotUI[0].ItemInSlot, Main.HoverItem))
                        newItem = enchantingTableUI.itemSlotUI[0].ItemInSlot;
        ...
        {
            if(UtilityMethods.debugging) ($"\\/UpdatePotionBuffs(" + newItem.S() + ", " + oldItem.S() + ")").Log();
        ...
        private void UpdatePotionBuff(ref Item item, bool remove = false)
        {
            ...
            if (WEMod.IsEnchantable(item))
            {
                ...
                {
                    if(UtilityMethods.debugging) ($"player: {buffs.S(key)}, item: {iGlobal.buffs.S(key)}").Log();
        ...
        {
            if(UtilityMethods.debugging) ($"\\/CombineStatModifier(baseStatModifier: " + baseStatModifier.S() + ", newStatModifier: " + newStatModifier.S() + ", remove: " + remove + ") StatModifier").Log();
            ...
            if(UtilityMethods.debugging) ($"/\\CombineStatModifier(baseStatModifier: " + baseStatModifier.S() + ", newStatModifier: " + newStatModifier.S() + ", remove: " + remove + ") return " + finalModifier.S()).Log();
            return finalModifier;
        }
        public static StatModifier InverseCombineWith(StatModifier baseStatModifier, StatModifier newStatModifier, bool remove)
        {
            if(UtilityMethods.debugging) ($"\\/InverseCombineWith(baseStatModifier: " + baseStatModifier.S() + ", newStatModifier: " + newStatModifier.S() + ", remove: " + remove + ") StatModifier").Log();
            ...
            if(UtilityMethods.debugging) ($"newInvertedStatModifier: " + newInvertedStatModifier.S()).Log();
            StatModifier finalModifier = baseStatModifier.CombineWith(newInvertedStatModifier);
            if(UtilityMethods.debugging) ($"/\\InverseCombineWith(baseStatModifier: " + baseStatModifier.S() + ", newStatModifier: " + newStatModifier.S() + ", remove: " + remove + ") return " + finalModifier.S()).Log();
            return finalModifier;
        }
        public static void TryRemoveStat(ref Dictionary<string, StatModifier> dictionary, string key)
        {
            if(UtilityMethods.debugging) ($"\\/TryRemoveStat( dictionary, key: " + key + ") dictionary: " + dictionary.S(key)).Log();
        ...
        {
            if(UtilityMethods.debugging) ($"\\/UpdatePlayerStats(" + newItem.S() + ", " + oldItem.S() + ")").Log();
        ...
        public void UpdateItemStats(ref Item item)
        {
            if (WEMod.IsEnchantable(item))
            {
                ...
                if (item.G().prefix != item.prefix)
                {
                    ...
                    DamageClass damageType = item.G().damageType;
                    if (damageType != null)
