...
using WeaponEnchantments.Content.Prefixes;
		...
		internal class ExamplePersonUI : UIState
		{
			...
			public override void OnDeactivate()
			{
				if (!_vanillaItemSlot.Item.IsAir)
				{
					...
					Main.LocalPlayer.QuickSpawnClonedItem(Main.LocalPlayer.GetItemSource_Misc(ItemSourceID.PlayerDropItemCheck), _vanillaItemSlot.Item, _vanillaItemSlot.Item.stack);
			...
			public override void Update(GameTime gameTime)
			{
				...
				if (Main.LocalPlayer.talkNPC == -1 || Main.npc[Main.LocalPlayer.talkNPC].type != ModContent.NPCType<ExamplePerson>())
				{
					...
					ModContent.GetInstance<WeaponEnchantments>().ExamplePersonUserInterface.SetState(null);
					...
					if (hoveringOverReforgeButton)
					{
						...
						if (Main.mouseLeftRelease && Main.mouseLeft && Main.LocalPlayer.CanBuyItem(awesomePrice, -1) && ItemLoader.PreReforge(_vanillaItemSlot.Item))
						{
							...
							if (Main.rand.NextBool(10))
							{
								...
								reforgeItem.Prefix(PrefixType<ExamplePrefix>());
							}
							else
							{
								//reforgeItem.Prefix(ModContent.GetInstance<ExampleMod>().PrefixType("Awesome"));
								reforgeItem.Prefix(ModConfig.PrefixType<ExampleDerivedPrefix>());
							}
							*/
