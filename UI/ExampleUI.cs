...
namespace WeaponEnchantments.UI
{
		...
		internal class ExampleUI : UIState
		{
			...
			public override void OnInitialize()
			{
				...
				Asset<Texture2D> buttonPlayTexture = ModContent.Request<Texture2D>("Terraria/UI/ButtonPlay");
				...
				Asset<Texture2D> buttonDeleteTexture = ModContent.Request<Texture2D>("Terraria/UI/ButtonDelete");
				...
				Asset<Texture2D> buttonFavoriteTexture = ModContent.Request<Texture2D>("Terraria/UI/ButtonFavoriteActive");
		...
		public class UIMoneyDisplay : UIElement
		{
			...
			protected override void DrawSelf(SpriteBatch spriteBatch)
			{
				...
				for (int j = 0; j < 4; j++)
				{
					...
					spriteBatch.Draw(TextureAssets.Item[74 - j].Value, new Vector2(shopx + 11f + (float)(24 * j), shopy /*+ 75f*/), null, Color.White, 0f, TextureAssets.Item[74 - j].Value.Size() / 2f, 1f, SpriteEffects.None, 0f);
					Utils.DrawBorderStringFourWay(spriteBatch, FontAssets.ItemStack.Value, coinsArray[3 - j].ToString(), shopx + (float)(24 * j) + (float)num, shopy/* + 75f*/, Color.White, Color.Black, new Vector2(0.3f), 0.75f);
				}
				...
				{
					int num = j == 0 && coinsArray[3 - j] > 99 ? -6 : 0;
					spriteBatch.Draw(TextureAssets.Item[74 - j].Value, new Vector2(shopx + 11f + (float)(24 * j), shopy + 25f), null, Color.White, 0f, TextureAssets.Item[74 - j].Value.Size() / 2f, 1f, SpriteEffects.None, 0f);
					Utils.DrawBorderStringFourWay(spriteBatch, FontAssets.ItemStack.Value, coinsArray[3 - j].ToString(), shopx + (float)(24 * j) + (float)num, shopy + 25f, Color.White, Color.Black, new Vector2(0.3f), 0.75f);
				}
				//Utils.DrawBorderStringFourWay(spriteBatch, WeaponEnchantments.exampleFont ?? FontAssets.ItemStack.Value, "CPM", shopx + (float)(24 * 4), shopy + 25f, Color.White, Color.Black, new Vector2(0.3f), 0.75f);
			}
		...
		public class MoneyCounterGlobalItem : GlobalItem
		{
			public override bool OnPickup(Item item, Player player)
			{
				if (item.type >= ItemID.CopperCoin && item.type <= ItemID.PlatinumCoin)
					ModContent.GetInstance<WeaponEnchantments>().ExampleUI.UpdateValue(item.stack * (item.value / 5));
