...
namespace WeaponEnchantments.Common.Globals
{
    public class WEGlobalTile : GlobalTile
    {
		...
		//						dropItem = TileTypeToItemType[type];
		//					}
		//					else if (GenericGlobalTile.tileTypeToItemTypesSetup && !ignoreError) {
		//						switch (modTile.Name) {
		//							case "SCalAltar":
		//								break;//No dropped item, don't log.
		//							default:
		//								$"Failed to determine the dropItem of tile: {type.GetTileIDOrName()}, modTile.Name: {modTile.Name}, modTile.ItemDrop: {itemDrop}.".LogNT_WE(ChatMessagesIDs.FailedDetermineDropItem);
		//								break;
		//						}
		//					}
		//				}
		//				else if (!ignoreError) {
		//					$"Failed to determine the dropItem of tile: {type.GetTileIDOrName()}.".LogNT_WE(ChatMessagesIDs.FailedDetermineDropItem);
		//				}
		//			}
