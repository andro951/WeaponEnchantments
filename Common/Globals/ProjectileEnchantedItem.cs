            ...
            if (target.life <= 0)//If NPC died
            {
                ...
                if (sourceItem.DamageType == DamageClass.Summon || sourceItem.DamageType == DamageClass.MagicSummonHybrid)//If item is a summoner weapon
                {
                    ...
                    else//If there is a previous known location, check that location in the player's inventory or banks
                    {
                        ...
                        if (inventory[inventoryLocation].type != sourceItem.type || inventory[inventoryLocation].GetGlobalItem<EnchantedItem>().enchantments != sourceItem.GetGlobalItem<EnchantedItem>().enchantments || wePlayer.Player.inventory[inventoryLocation].value != sourceItem.value || inventory[inventoryLocation].GetGlobalItem<EnchantedItem>().powerBoosterInstalled != sourceItem.GetGlobalItem<EnchantedItem>().powerBoosterInstalled)
                        {
                            found = false;
                        }
                        else
                            ...
                            if (!inventory[inventoryLocation].IsAir)
                            {
                                if (inventory[inventoryLocation].type == sourceItem.type)
                                {
                                    if (inventory[inventoryLocation].GetGlobalItem<EnchantedItem>().powerBoosterInstalled == sourceItem.GetGlobalItem<EnchantedItem>().powerBoosterInstalled)
                                    {
                                        ...
                                        {
                                            if (inventory[inventoryLocation].GetGlobalItem<EnchantedItem>().enchantments == sourceItem.GetGlobalItem<EnchantedItem>().enchantments)
                                            {
